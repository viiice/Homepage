(function () {
    ...
    var flightMove = document.getElementById('flight_a')
    ...
    function moveflight() {
        if (w_num <= 400) {
            // flightMove.css({"left":x_arr[w_num]+"px","top":y_arr[w_num]});
            flightMove.style.left = x_arr[w_num] + 'px';
            flightMove.style.top = y_arr[w_num] + 'px';
            w_num += 3;
        }
        else {
    ...
    moveflight();
    var weiTimer = setInterval(moveflight, 1);
    moveblog();
